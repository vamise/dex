...
export default function DeprecatedMarketsInstructions({ switchToLiveMarkets }) {
    ...
    return ( <
        ...
        Wrapped USDC <
        /Link>, now called WUSDC, to Native USDC. You can convert from WUSDC to USDC directly on { ' ' } <
        Link external to = { 'https://sollet.io' } >
        sollet.io <
        /Link>, or can deposit WUSDC and be credited USDC on exchanges like { ' ' } <
        ...
        <
        Typography >
        Markets with WUSDC as the quote currency are now deprecated.To migrate over to the new USDC markets, please cancel your orders and settle your funds on old markets. <
        /Typography> <
        div style = {
        ...
        div > {!balances ? ( <
                Spin size = "large" / >
                    ...
                    >
                    <
                    Divider > {!needToCancelOrders && < CheckOutlined / > }
                    Cancel your orders <
                    /Divider> {
                    needToCancelOrders ? (
                        loaded ? ( <
                ...
                Divider > {!needToSettleFunds && < CheckOutlined / > }
            Settle your funds <
            /Divider> {
            needToSettleFunds && ( <
                BalancesTable balances = { filteredBalances }
                hideWalletBalance showMarket onSettleSuccess = {
                    () => {
        ...
        <
        Button onClick = { switchToLiveMarkets }
